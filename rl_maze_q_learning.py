# -*- coding: utf-8 -*-
"""RL_maze_Q-Learning.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1xEkjEA4-i5h7pcHKbEYYt4yVbUUAorv7
"""

# Commented out IPython magic to ensure Python compatibility.
# Modification from "https://github.com/YutaroOgawa/Deep-Reinforcement-Learning-Book/blob/master/program/2_6_Qlearning.ipynb"
'''
...
# Q-Learning, renewing the quality function Q 

def Q_learning(s, a, r, s_next, Q, eta, gamma):

    if s_next == 99:  # When reaching the end point
...
# Q-Learningで迷路を解く関数の定義、状態と行動の履歴および更新したQを出力
...
is_continue = True
episode = 1
current_actions_total_steps = .0
current_actions_total_steps_history = []
...
while is_continue:  # is_continueがFalseになるまで繰り返す
    ...
    v_diff = np.sum(np.abs(new_v - v))
    v_diff_history.append(v_diff)
    current_actions_total_steps = len(s_a_history) - 1
    current_actions_total_steps_history.append(np.abs(current_actions_total_steps))
    print( str(episode) + " : ", v_diff ,"\t  The total steps used to finish the maze is ", current_actions_total_steps )
    v = new_v
...
#anim
anim.save("maze_RL_Q_learning.mp4")

#plt.plot(v_diff_history)

#plt.plot(np.log(v_diff_history/v_diff_history[-1]))

#plt.plot(current_actions_total_steps_history)

#current_actions_total_steps_history[-1]

#new_list = [x/current_actions_total_steps_history[-1] for x in current_actions_total_steps_history]
#new_list_log = np.log(new_list)
#plt.plot(new_list_log)
...
def animate(i):
    ...
    # Draw the text to represent state of agent in the maze
    def plot_fun(i,xid,yid):
        state_id = xid + 10*(yid)
        if state_id == 99:
            color_map = cm.jet(1.0)
        else:
            color_map = cm.jet(V[i][state_id])
            
        line, = ax.plot([0.5+xid], [9.5-yid], marker="s",color=color_map,
                        markersize=65)

    [[plot_fun(i,xid,yid) for xid in range(10)] for yid in range(10)]
...
#rc('animation', html='jshtml')
#anim
print("\nstart_to_save_V_history !")
anim.save("maze_RL_Q_learning_V_history.mp4")
